...
const EmailForm = (props: any) => {
  ...
  useEffect(() => {
    if (registerError.payload) {
      if (
        REGISTER_ERROR_RESPONSE.EMAIL_NOT_AVAILABLE === registerError.payload.data?.errors?.contact
      ) {
        setError((prevState) => {
          return {
            ...prevState,
            email: registerError.payload.data?.errors?.contact,
          };
        });
      }
    }
  ...
  const validateData = () => {
    ...
    if (!data.email) {
      ...
      if (!Regex.isEmailValid(data.email)) {
        setError((prevState) => {
          return {
            ...prevState,
  ...
  return (
    <div className="mt-8">
      ...
      <TextInput
        ...
        className={`mb-2 w-full rounded border border-neutral-900 px-4 py-3 font-poppins text-[16px] font-regular
          text-[var(--black500)] placeholder-[var(--black500)] outline-none
          ${error.email && '!border-[var(--red500)]'}
        `}
      />
      ...
      >
        Please enter a valid email address. You will receive a confirmation mail associated with
        this account. Please make sure to check your incoming email from us.
      </p>
      {/* End of email address */}
...
const WrappedComponent = compose(withRegisterHoc, withRegisterSelector)(EmailForm);
